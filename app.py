...
def Push_data_to_sql(names):
    ...
    Projection = records.find({"Chennal_Inforamtion.Chennal_Name":names},{"_id":0})
    ...
    for index,row in df.iterrows():
        ...
        try:
            ...
            error = (f"Datas from {names} are migrated to SQL already")
    return  error

def Push_video_data_sql(names):
        mycursor.execute("USE YouTube_Data")
        ...
        db = client.YouTube_Warehouse
        records = db.Chennal_Details
        Projection = records.find({"Video_Information.Chennal_Name":names},{"_id":0})
...
def Push_playlist_data_sql(names):
    mycursor.execute("USE YouTube_Data")
    try:
    ...
    db = client.YouTube_Warehouse
    records = db.Chennal_Details
    Projection = records.find({"Playlist_information.Chennal_Name":names},{"_id":0})
...
def Push_comments_data_sql(names):
    mycursor.execute("USE YouTube_Data")
    try:
...
def unique_ch():
    ...
    for coll in Projection:
        Ch_list.append(coll['Chennal_Inforamtion']['Chennal_Name'])
    return Ch_list


def create_tabels(single_channel):
    error = Push_data_to_sql(single_channel)
    if error:
        return error
    else:
        Push_video_data_sql(single_channel)
        Push_playlist_data_sql(single_channel)
        Push_comments_data_sql(single_channel)
...
if selected == "⚙️ Fetch and Export Data":
    ...
    if not url_input:
        ...
        name = unique_ch()
        unique_ch_names = st.selectbox("Select a Channel to Migrate", name)
        
        if st.button("Migrate data to SQL data warehouse", type = "primary" ):
            sqltable = create_tabels(unique_ch_names)
